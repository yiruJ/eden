...
const ModelInner = ({
  ...
  const texture = customTexture ? useLoader(THREE.TextureLoader, customTexture) : null;
    ...
    if (texture) {
      ...
      g.traverse((o) => {
        if ((o as THREE.Mesh).isMesh) {
          ...
          if (Array.isArray(mesh.material)) mesh.material.forEach(applyTex);
          else applyTex(mesh.material);
        }
      });
    }
    ...
    <div style={{ width, height, touchAction: 'pan-y pinch-zoom', position: 'relative' }}>
      ...
      <Canvas
        ...
        onCreated={({ gl, scene, camera }) => {
          (rendererRef as React.MutableRefObject<THREE.WebGLRenderer>).current = gl;
          (sceneRef as React.MutableRefObject<THREE.Scene>).current = scene;
      ...
      >
        {environmentPreset !== 'none' && (
          <Environment preset={environmentPreset as Parameters<typeof Environment>[0]['preset']} background={false} />
        )}
